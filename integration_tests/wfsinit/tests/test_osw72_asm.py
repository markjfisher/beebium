...
@pytest.mark.slow
@pytest.mark.timeout(120)
def test_hello_asm_boot_with_tube(
    server_filepath, mos_filepath, basic_filepath,
    anfs_filepath, adfs_filepath, dfs_filepath,
    scsi_hdd_filepath,
):
    """Minimal test: boot a 'HELLO DONE' program from SSD with Tube active.

    No SCSI operations, no ADFS. Just verifies that Shift-Break auto-boot
    can load and run a trivial program with the Tube enabled.
    """
    ...
    try:
        with Beebium.launch(
            ...
            if not ok:
                print(f"Screen:\n{screen}")
                _dump_hang_diagnostics(bbc)
                pytest.fail("Hello program did not complete. See diagnostics.")
...
@pytest.mark.slow
@pytest.mark.timeout(120)
def test_hello_asm_boot_without_tube(
    server_filepath, mos_filepath, basic_filepath,
    anfs_filepath, adfs_filepath, dfs_filepath,
    scsi_hdd_filepath,
):
    """Same hello test but WITHOUT the Tube -- isolates disc image issues."""
    ssd_filepath = ASM_DIRPATH / "test_hello.ssd"
    if not ssd_filepath.exists():
        pytest.skip(f"Test SSD not found: {ssd_filepath}")
    ...
    try:
        with Beebium.launch(
            ...
            print(f"Screen:\n{screen}")
            if not ok:
                pytest.fail("Hello program did not complete without Tube.")
