...
public:
    ...
    // Deferred read result: true after try_complete_stretch() resolves a
    // read stretch. The caller must retrieve the value and patch cpu.dbus.
    bool has_completed_read() const { return completed_read_pending_; }
    uint8_t take_completed_read_result() {
        completed_read_pending_ = false;
        return completed_read_result_;
    }
...
private:
    ...
    // Deferred read result: set by try_complete_stretch() when a read
    // stretch clears. The caller retrieves the value to patch cpu.dbus.
    uint8_t completed_read_result_ = 0;
    bool completed_read_pending_ = false;
