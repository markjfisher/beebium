...
QString sanitizeDockId(const QString &value) {
    QString dockId = value;
    dockId.replace(QRegularExpression(QStringLiteral("[^A-Za-z0-9_-]")), QStringLiteral("-"));
    return dockId;
}
...
MainWindow::MainWindow(const ConnectionTarget &initialTarget, QWidget *parent)
    ...
    serialConnectorLabel_ = new QLabel(tr("Unknown"), this);
    serialStateLabel_ = new QLabel(tr("Not connected"), this);
    serialLineStateLabel_ = new QLabel(tr("No signal state available"), this);
    serialRegistersLabel_ = new QLabel(tr("ACIA 0x00 / ULA 0x00"), this);
...
void MainWindow::buildDockLayout() {
    ...
    addPanelDock(QStringLiteral("serial"), tr("Serial Hardware"), createSerialPanel(), QStringLiteral("bottom"));
...
void MainWindow::buildMenus() {
    ...
    windowsMenu_ = viewMenu->addMenu(tr("Windows"));
    for (auto it = docks_.cbegin(); it != docks_.cend(); ++it) {
#if defined(BEEBIUM_HAVE_KDDOCKWIDGETS)
        QAction *action = it.value()->toggleAction();
        windowsMenu_->addAction(action);
        dockWindowActions_.insert(it.key(), action);
#else
        dockWindowActions_.insert(it.key(), windowsMenu_->addAction(it.value()->toggleViewAction()));
#endif
    }
...
QWidget *MainWindow::createSerialPanel() {
    ...
    auto *notes = new QLabel(
        tr("This panel reflects only the BBC's on-board serial hardware. Runtime device configuration now comes from extension panels discovered from the running server."),
        panel);
    notes->setWordWrap(true);
    layout->addRow(notes);
    layout->addRow(tr("Connector"), serialConnectorLabel_);
    layout->addRow(tr("BBC serial"), serialStateLabel_);
    layout->addRow(tr("Lines"), serialLineStateLabel_);
    layout->addRow(tr("Registers"), serialRegistersLabel_);
    return panel;
}
...
void MainWindow::addPanelDock(const QString &dockId, const QString &title, QWidget *widget, const QString &locationHint) {
    ...
    if (windowsMenu_) {
#if defined(BEEBIUM_HAVE_KDDOCKWIDGETS)
        QAction *action = docks_.value(dockId)->toggleAction();
        windowsMenu_->addAction(action);
        dockWindowActions_.insert(dockId, action);
#else
        dockWindowActions_.insert(dockId, windowsMenu_->addAction(docks_.value(dockId)->toggleViewAction()));
#endif
    }
}

void MainWindow::removePanelDock(const QString &dockId) {
    if (QAction *action = dockWindowActions_.take(dockId)) {
        if (windowsMenu_) {
            windowsMenu_->removeAction(action);
        }
        delete action;
    }

#if defined(BEEBIUM_HAVE_KDDOCKWIDGETS)
    if (auto *dock = docks_.take(dockId)) {
        delete dock;
    }
#else
    if (auto *dock = docks_.take(dockId)) {
...
void MainWindow::refreshExtensionUiPanels(const ConnectionTarget &target) {
    auto channel = grpc::CreateChannel(target.address().toStdString(), grpc::InsecureChannelCredentials());
    auto peripheralStub = beebium::PeripheralExtensionService::NewStub(channel);
    auto transportStub = beebium::EconetTransportService::NewStub(channel);

    QHash<QString, QString> wantedPanels;

    grpc::ClientContext extensionsContext;
    beebium::ListExtensionsRequest extensionsRequest;
    beebium::ListExtensionsResponse extensionsResponse;
    const grpc::Status extensionsStatus = peripheralStub->ListExtensions(&extensionsContext, extensionsRequest, &extensionsResponse);
    if (!extensionsStatus.ok()) {
        showError(tr("ListExtensions failed: %1").arg(QString::fromStdString(extensionsStatus.error_message())));
    } else {
        for (const auto &extension : extensionsResponse.extensions()) {
            if (extension.has_ui()) {
                const QString extensionId = QString::fromStdString(extension.id());
                const QString label = QString::fromStdString(extension.label());
                wantedPanels.insert(extensionId, label.isEmpty() ? extensionId : label);
            }
        }
    }

    grpc::ClientContext transportsContext;
    beebium::ListTransportsRequest transportsRequest;
    beebium::ListTransportsResponse transportsResponse;
    const grpc::Status transportsStatus = transportStub->ListTransports(&transportsContext, transportsRequest, &transportsResponse);
    if (transportsStatus.ok()) {
        for (const auto &transport : transportsResponse.transports()) {
            if (transport.has_ui()) {
                const QString transportId = QString::fromStdString(transport.id());
                const QString transportName = QString::fromStdString(transport.name());
                wantedPanels.insert(transportId, transportName.isEmpty() ? transportId : transportName);
            }
        }
    }

    const QStringList existingIds = extensionPanels_.keys();
    for (const QString &extensionId : existingIds) {
        if (wantedPanels.contains(extensionId)) {
            continue;
        }
        if (ExtensionUiPanel *panel = extensionPanels_.take(extensionId)) {
            panel->disconnectFromTarget();
            panel->deleteLater();
        }
        removePanelDock(extensionPanelDockIds_.take(extensionId));
    }

    for (auto it = wantedPanels.cbegin(); it != wantedPanels.cend(); ++it) {
        const QString extensionId = it.key();
        const QString label = it.value();
        ExtensionUiPanel *panel = extensionPanels_.value(extensionId);
        if (!panel) {
            panel = new ExtensionUiPanel(extensionId, label, this);
            connect(panel, &ExtensionUiPanel::errorOccurred, this, &MainWindow::showError);
            const QString dockId = QStringLiteral("extension-") + sanitizeDockId(extensionId);
            extensionPanels_.insert(extensionId, panel);
            extensionPanelDockIds_.insert(extensionId, dockId);
            addPanelDock(dockId, label, panel, QStringLiteral("right"));
        }
        panel->connectToTarget(target);
    }
}

void MainWindow::clearExtensionUiPanels() {
    const QStringList extensionIds = extensionPanels_.keys();
    for (const QString &extensionId : extensionIds) {
        if (ExtensionUiPanel *panel = extensionPanels_.take(extensionId)) {
            panel->disconnectFromTarget();
            panel->deleteLater();
        }
...
void MainWindow::connectToServer() {
    ...
    refreshExtensionUiPanels(target);
...
void MainWindow::refreshSerialView() {
    ...
    if (!status.hasSerialSocket) {
        serialConnectorLabel_->setText(tr("Not fitted"));
        serialStateLabel_->setText(tr("Machine has no serial socket"));
        serialLineStateLabel_->setText(tr("No serial line signals available"));
        serialRegistersLabel_->setText(tr("ACIA 0x00 / ULA 0x00"));
        return;
    }

    serialConnectorLabel_->setText(status.connector.isEmpty() ? tr("Unknown") : status.connector);
    serialStateLabel_->setText(
        tr("TX %1 baud, RX %2 baud (%3)")
            .arg(status.txBaud)
            .arg(status.rxBaud)
            .arg(status.rs423Selected ? tr("serial selected") : tr("cassette selected")));
    serialLineStateLabel_->setText(
        tr("/CTS %1, /DCD %2, IRQ %3, TDRE %4, RDRF %5, Motor %6")
            .arg(status.notCts ? tr("inactive") : tr("active"))
            .arg(status.notDcd ? tr("inactive") : tr("active"))
            .arg(status.irqPending ? tr("pending") : tr("clear"))
            .arg(status.tdre ? tr("set") : tr("clear"))
            .arg(status.rdrf ? tr("set") : tr("clear"))
            .arg(status.motorOn ? tr("on") : tr("off")));
    serialRegistersLabel_->setText(
        tr("ACIA 0x%1 / ULA 0x%2 (TX bit %3, RX bit %4)")
            .arg(status.aciaControl, 2, 16, QLatin1Char('0'))
            .arg(status.ulaControl, 2, 16, QLatin1Char('0'))
            .arg(status.txBitPeriod)
            .arg(status.rxBitPeriod));
}
