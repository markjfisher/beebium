...
# Generated stubs live in the core package and in each extension package.
PROTO_OUT_DIRS=(src/beebium/client/_proto src/beebium/ext)
...
if ! git diff --quiet -- "${PROTO_OUT_DIRS[@]}"; then
    echo
    ...
    echo "result. Diff:" >&2
    echo
    git --no-pager diff --stat -- "${PROTO_OUT_DIRS[@]}" >&2
    git --no-pager diff -- "${PROTO_OUT_DIRS[@]}" >&2
    exit 1
fi
...
untracked="$(git ls-files --others --exclude-standard -- "${PROTO_OUT_DIRS[@]}")"
if [[ -n "$untracked" ]]; then
    echo
