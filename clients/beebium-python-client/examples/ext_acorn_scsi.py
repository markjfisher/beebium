...
def demo(bbc: Beebium) -> None:
    ...
    print(
        f"bus: phase={status.phase} selected={selected} status=${status.status_register:02X} irq={status.irq_pending}"
    )


if __name__ == "__main__":
    run(demo, description="acorn-scsi target and bus inspection.", extra_args=["--acorn-scsi"])
