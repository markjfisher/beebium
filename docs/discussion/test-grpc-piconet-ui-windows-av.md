...
- Removing `serial_factory_` as a `std::function` (workaround #6
  above) avoids the bug at the call site but doesn't address the
  `notify_state_changed` → `on_async_state_change_` path, which
  exhibits the same crash signature in earlier traces. So this is a
  partial workaround at best.
