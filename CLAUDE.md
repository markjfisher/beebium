...
Write tests before implementation. The test suite is comprehensive (~65 test files covering 6502 instructions, VIA timing, disc operations, sound chip behaviour, video rendering, keyboard input, and gRPC services). When adding new functionality:
...
- Service advertisement over mDNS on all platforms (macOS Bonjour, Linux Avahi, Windows DNS-SD); browse/discovery client on macOS (Bonjour) and Windows (DnsServiceBrowse/Resolve); Linux still uses NullBrowser
- Python test client
